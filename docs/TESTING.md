...
- Python 3.13.13: `python -m unittest discover -s tests -v` passes all 30 tests.
...
- Ubuntu 26.04 WSL with Python 3.14.4: 29 tests passed and the Windows API test was skipped. Mock tests check logind availability responses and D-Bus method selection without interactive authentication.
- Linux desktop GTK integration, actual inhibitor behavior, sleep, hibernation, and shutdown are **NOT TESTED**. WSL does not replace physical-device validation.
...
4. Check the countdown, Cancel button, and closing the countdown window. Run again and wait for the dry-run completion message; power state must not change.
